...
regional.spectra <- lapply(regional.spectra, function(x) {LogSmooth(x, df.log=smooth, removeLast=2)})
global.spectra <- lapply(global.spectra, function(x) {LogSmooth(x, df.log=smooth, removeLast=2)})

zonal.spectra_smooth <- list()
for(i in names(zonal.spectra)){
  zonal.spectra_smooth[[i]] <- lapply(zonal.spectra[[i]], function(x) {LogSmooth(x, df.log=smooth, removeLast=2)}) #0
}
...
#---PLOT---#
saveToPDF <- F
filenam <- "03_spectral_analysis/f3_sat_hadcm3_lats_revised.pdf"

if(saveToPDF) pdf(file=filenam,width=10,height=7)
  
layout(matrix(c(1,1,2,3),2,2),widths=c(1,0.75),heights=c(1.,1))

par(mar=c(0,2.,0.,0.), oma=c(2.6,0.8,0,0.2), cex=1) #bottom, left, top, right
...
text(1/3, 0.8, "Local")
text(1/3, 0.02, "Global")
  
axis(1,labels=FALSE)
mtext(side=1,at=axTicks(1),1/axTicks(1),line=0.5,cex=cex.t,lwd=lwd.t)
mtext(side=1,"Period [years]",line=1.5,cex=cex.t,lwd=lwd.t)
  
axis(2, at=axTicks(2)[c(2,3,5,6,8,9, 11, 12)], tick=axTicks(2)[c(2,3,5,6,8,9,11,12)], labels=FALSE)
mtext(side=2,at=axTicks(2)[seq(3,length(axTicks(2)),by=3)]
        , axTicks(2)[seq(3,length(axTicks(2)),by=3)],line=0.5,cex=cex.t,lwd=lwd.t)
...
type = "lgm"
    ...
    axis(2, at=axTicks(2)[c(2,3,5,6,8,9)], labels=FALSE)
    ...
    legend("topright"
           ...
           ,col=COL[order]
           ,lwd=lwd.t
           ,legend =  c("trop", "mid (N)", "mid (S)", "high (N)", "high (S)")[order],
